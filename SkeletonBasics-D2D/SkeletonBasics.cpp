...
void CSkeletonBasics::ProcessSkeleton()
{
    ...
    for (int i = 0 ; i < NUI_SKELETON_COUNT; ++i)
    {
        ...
        else if (NUI_SKELETON_POSITION_ONLY == trackingState)
        {
			...
			m_pD2DRenderTarget->DrawEllipse(ellipse, m_pBrushJointTracked);
        }
    }
...
{      
    ...
    for (i = 0; i < NUI_SKELETON_POSITION_COUNT; ++i)
    {
        ...
        if ( skel.eSkeletonPositionTrackingState[i] == NUI_SKELETON_POSITION_INFERRED )
        {
			...
			m_pD2DRenderTarget->DrawEllipse(ellipse, m_pBrushJointInferred);
        }
...
void CSkeletonBasics::DrawBone(const NUI_SKELETON_DATA & skel, NUI_SKELETON_POSITION_INDEX joint0, NUI_SKELETON_POSITION_INDEX joint1)
{
    ...
    if (joint0State == NUI_SKELETON_POSITION_TRACKED && joint1State == NUI_SKELETON_POSITION_TRACKED)
    {
		...
		m_pD2DRenderTarget->DrawLine(m_Points[joint0], m_Points[joint1], m_pBrushBoneTracked, g_TrackedBoneThickness);
    }
    else
    {
        m_pRenderTarget->DrawLine(m_Points[joint0], m_Points[joint1], m_pBrushBoneInferred, g_InferredBoneThickness);
		m_pD2DRenderTarget->DrawLine(m_Points[joint0], m_Points[joint1], m_pBrushBoneInferred, g_InferredBoneThickness);
    }
}
...
HRESULT CSkeletonBasics::EnsureDirect2DResources()
{
	...
	if (NULL == m_pD2DRenderTarget) {
		...
		rtProps.pixelFormat = D2D1::PixelFormat(DXGI_FORMAT_B8G8R8A8_UNORM, D2D1_ALPHA_MODE_IGNORE);
		rtProps.usage = D2D1_RENDER_TARGET_USAGE_GDI_COMPATIBLE;
		hrBit = iwicImage->CreateBitmap(width, height, GUID_WICPixelFormat24bppRGB, WICBitmapCacheOnDemand, &wicBM);
		hrBit = m_pD2DFactory->CreateWicBitmapRenderTarget(wicBM, rtProps, &m_pD2DRenderTarget);


		

		m_pD2DRenderTarget->CreateSolidColorBrush(D2D1::ColorF(0.27f, 0.75f, 0.27f), &m_pBrushJointTracked);

		m_pD2DRenderTarget->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::Yellow, 1.0f), &m_pBrushJointInferred);
		m_pD2DRenderTarget->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::Green, 1.0f), &m_pBrushBoneTracked);
		m_pD2DRenderTarget->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::Gray, 1.0f), &m_pBrushBoneInferred);

		if (FAILED(hrBit))
		{
			SetStatusMessage(L"Couldn't create Direct2D bitman render target!");
			return hr;
		}
	}
